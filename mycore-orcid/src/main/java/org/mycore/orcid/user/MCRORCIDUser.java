...
import org.mycore.user2.MCRUserAttributeInt;
...
public class MCRORCIDUser {
    ...
    public void store(MCRTokenResponse token) {
        user.getAttributes().add(new MCRUserAttributeInt(ATTR_ORCID_ID, token.getORCID()));
        user.getAttributes().add(new MCRUserAttributeInt(ATTR_ORCID_TOKEN, token.getAccessToken()));
    ...
    private Set<String> getUserIdentifierKeys() {
        ...
        for (MCRUserAttributeInt attribute : user.getAttributes()) {
